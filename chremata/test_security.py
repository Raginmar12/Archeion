...
class DeviceTokenSecurityTests(TestCase):
    ...
    @override_settings(DEBUG=True, ARCHEION_DEVICE_TOKEN="token-secreto")
    def test_token_configurado_se_exige_incluso_en_debug(self):
        response = self.client.get(reverse("device-ping"))

        self.assertEqual(response.status_code, 401)

    @override_settings(DEBUG=False, ARCHEION_DEVICE_TOKEN="token-secreto")
    ...
    def test_token_valido_permite_api(self):
        response = self.client.get(
            reverse("device-ping"),
            headers={"X-Archeion-Device-Token": "token-secreto"},
        )
