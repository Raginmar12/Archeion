# Seguridad local de Archeion

## Tokens de dispositivo

Las rutas bajo `/api/` requieren autenticación mediante el encabezado
`X-Archeion-Device-Token`. Cada Cardputer, script local o módulo satélite debe usar su
...
después. Archeion guarda únicamente su hash SHA-256 y un prefijo identificador; nunca
...
Mientras exista, `ARCHEION_DEVICE_TOKEN` funciona únicamente como fallback temporal o de
...
Archeion es un sistema local-first y no debe exponerse directamente a internet. Si en el
futuro se requiere acceso remoto, debe utilizarse una red privada como Tailscale,
WireGuard u otra VPN, sin abrir puertos públicos directamente hacia Archeion.
